...
def compute_quality_flags(
    ...
    # Новая эвристика: проверка на константные колонки
    constant_columns = 0
    for col in summary.columns:
        if col.unique == 1 and col.non_null > 0:  # Если все значения одинаковые
            constant_columns += 1
    flags["has_constant_columns"] = constant_columns > 0
    
    # Новая эвристика: проверка на большое количество нулевых значений
    zero_values_share_total = 0.0
    numeric_columns_count = 0
    for col in summary.columns:
        if col.is_numeric and col.non_null > 0:
            numeric_columns_count += 1
            # Считаем долю нулевых значений
            zero_count = 0
            for val in col.example_values:
                if str(val) == '0' or str(val) == '0.0':
                    zero_count += 1
            if zero_count > 0:
                zero_share = zero_count / len(col.example_values)
                zero_values_share_total = max(zero_values_share_total, zero_share)
    
    flags["has_many_zero_values"] = zero_values_share_total > max_zero_share
