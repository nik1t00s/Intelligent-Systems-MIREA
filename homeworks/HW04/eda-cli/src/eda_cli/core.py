...
def compute_quality_flags(
    ...
    # 2. Проверка на большое количество нулевых значений
    zero_values_share_total = 0.0
    numeric_columns_count = 0
    for col in summary.columns:
        if col.is_numeric and col.non_null > 0:
            numeric_columns_count += 1
            # Считаем долю нулевых значений в числовых колонках
            if "0" in [str(v) for v in col.example_values] or 0 in col.example_values:
                # Это упрощенная проверка для примера
                zero_values_share_total = max(zero_values_share_total, 0.3)
